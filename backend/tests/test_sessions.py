...
def test_create_session(client, test_user, db_session):
    # Valid creation
...
def test_create_session_unauthorized(client, db_session):
    # No user header
    response = client.post(
        "/api/chatbot/sessions",
        json={"title": "Test"}
...
def test_list_sessions(client, test_user, db_session):
    ...
    response = client.get("/api/chatbot/sessions", headers={"X-User-Id": str(test_user.id)})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
...
def test_get_session(client, test_user, db_session):
    s1 = ChatbotSession(user_id=test_user.id, title="S1")
    db_session.add(s1)
    db_session.commit()
    
    response = client.get(f"/api/chatbot/sessions/{s1.id}", headers={"X-User-Id": str(test_user.id)})
    assert response.status_code == 200
    data = response.json()
...
def test_get_nonexistent_session(client, test_user, db_session):
    import uuid
    random_id = uuid.uuid4()
    response = client.get(f"/api/chatbot/sessions/{random_id}", headers={"X-User-Id": str(test_user.id)})
    assert response.status_code == 404

def test_session_ownership(client, test_user, db_session):
    ...
    response = client.get(f"/api/chatbot/sessions/{s_other.id}", headers={"X-User-Id": str(test_user.id)})
    assert response.status_code == 404

def test_delete_session(client, test_user, db_session):
    s1 = ChatbotSession(user_id=test_user.id, title="To Delete")
    db_session.add(s1)
    db_session.commit()
    
    response = client.delete(f"/api/chatbot/sessions/{s1.id}", headers={"X-User-Id": str(test_user.id)})
    assert response.status_code == 200
    
    # Verify deletion
    verify = client.get(f"/api/chatbot/sessions/{s1.id}", headers={"X-User-Id": str(test_user.id)})
