...
async def mock_stream_chunks(messages, model):
    if model == 'llama-3.3-70b-versatile':
        yield (True, 'data: {"content": "First partial response"}\n\n')
        from groq import RateLimitError
        import httpx
        raise RateLimitError('Rate limited mid stream', response=httpx.Response(429, request=httpx.Request('GET', 'http://127.0.0.1')), body=None)
    else:
